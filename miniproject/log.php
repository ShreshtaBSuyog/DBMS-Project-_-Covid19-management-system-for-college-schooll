...
<nav class="navbar navbar-inverse">
  ...
  <div class="row content">
    <div class="col-sm-2 sidenav">
      ...
      <h2 style="color:blue;text-decoration: underline;">Infection Report Log  Details</h2> 
...
$sql="SELECT * FROM stulogs";
    $result=mysqli_query($db,$sql) or die("Bad query:");  
while($row=mysqli_fetch_assoc($result)){
 echo "<tr><td>{$row['id']}</td><td>{$row['stu_id']}</td><td>{$row['name']}</td><td>{$row['action']}</td><td><b>{$row['udate']}</b></td></tr>"; 
}
?>
    </tbody>
</table>
